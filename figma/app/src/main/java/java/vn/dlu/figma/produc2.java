...
public class produc2 extends AppCompatActivity implements View.OnClickListener {
    ...
    private void handleCommonFunctionality(int viewId) {
        if (viewId == R.id.buy) {
            startNewActivity(thanhtoan_vat.class);}
        if (viewId == R.id.read) {
    ...
    private void startNewActivity(Class<?> destinationClass) {
        startActivity(new Intent(produc2.this, destinationClass));
